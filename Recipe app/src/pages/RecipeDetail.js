...
const RecipeDetail = () => {
  ...
  const getRecipe = async (id) => {
    try {
      ...
      const favorited = storedFavorites.some((fav) => fav.uri === data.uri);
  ...
  const handleFavorite = () => {
    ...
    if (isFavorited) {
      ...
      const updatedFavorites = storedFavorites.filter((fav) => fav.uri !== recipe.uri);
  ...
  if (!recipe) {
    return (
      <div className='w-full h-[100vh] flex items-center justify-center'>
        <p className="text-white">Recipe not found or there was an error loading the recipe.</p>
      </div>
    );
  }
  return (
    <div className='w-full'>
      ...
      <div className='w-full flex items-center justify-center mt-4'>
        <button
          ...
          className='flex items-center gap-2 bg-transparent border border-white text-white hover:bg-white hover:text-black rounded-full px-4 py-2'
        >
      ...
      <div className='w-full px-4 lg:px-20 pt-5'>
        ...
        <div className='w-full flex flex-col md:flex-row gap-8 py-20 pxx-4 md:px-10'>
          ...
          <div className='w-full md:w-2/4 md:border-r border-slate-800 pr-1'>
            <div className='flex flex-col gap-5'>
              <p className='text-pink-500 text-2xl underline'>Ingredients</p>

              {recipe?.ingredientLines?.map((ingredient, index) => {
                return (
                  <p key={index} className='text-neutral-100 flex gap-2'>
                    <AiFillPushpin className='text-pink-800 text-xl' /> {ingredient}
                  </p>
                );
              })}
            </div>
            ...
            <div className='flex flex-col gap-3 mt-20'>
              <p className='text-pink-500 text-2xl underline'>Health Labels</p>
  ...
  recipe ? <RecipeCard recipe={recipe} key={index} /> : null
))}
                </div>
              </>
            )}
